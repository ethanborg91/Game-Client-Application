...
# This player class is responsible for holding the room information
# and getting request from the player and dealing with them appropriately

# The address for the discovery socket
...
def signal_handler(sig, frame):
    ...
    # Tells the discovery to deregister the room

    room_socket.sendto(message.encode(), DISCOVERYSERVER)
...
def client_list_except_player(player):
    players = []
    for reg in client_list:
        if reg[0] != player and player != None:
            players.append(reg)
    return players
...
def process_message(message, addr, room_socket):
    ...
    # If the discovery class has closed down it will close all rooms and players

    elif message == 'discovery closed':
        print('Discovery has closed, shutting down ...')
        message='disconnect'
...
def main():
    ...
    if args.n != "":
        connections["north"] = args.n
    if args.s != "":
        connections["south"] = args.s
    if args.e != "":
        connections["east"] = args.e
    if args.w != "":
        connections["west"] = args.w
    if args.u != "":
        connections["up"] = args.u
    if args.d != "":
        connections["down"] = args.d
    ...
    room_socket.bind(('', 0))
    word = "REGISTER " + str(room_socket.getsockname()[1]) +  " " + name
    room_socket.sendto(word.encode(), DISCOVERYSERVER)

    message, addr = room_socket.recvfrom(1024)

    if message.decode() != 'OK':
        print("There is already a room with the same name")
        sys.exit(0)
